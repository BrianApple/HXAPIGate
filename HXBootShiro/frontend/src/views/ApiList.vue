    ...
    <el-table :data="apis" v-loading="loading" border stripe>
      ...
      <el-table-column label="协议" width="150">
        <template #default="{ row }">
          <el-tag size="small" :type="protoType(row) === 'mcp' ? 'warning' : protoType(row) === 'websocket' ? 'primary' : ''">
            {{ protoType(row) === 'dubbo' ? 'Dubbo' : protoType(row) === 'mcp' ? 'MCP' : protoType(row) === 'websocket' ? 'WebSocket' : 'HTTP' }}
          </el-tag>
