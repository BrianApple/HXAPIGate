...
- 接口同一uri+version,不可重复
- 所有未被做任何授权的API,默认关联的角色为"00_no_auth_role"，该角色代码中，不在数据库中存储，只要API重新被授权，则"00_no_auth_role"的授权关系会被擦除
- 重要： HXBootshiro所有需要对外安全提供服务的API，都需要添加到API管理平台中，没有被管理起来的API接口，无法被API网关代理，同时，
没有被管理起来的API，将可以被直接访问，无任何保护措施！！！！
